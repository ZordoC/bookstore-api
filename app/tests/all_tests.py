...
def test_token_unsuccessful():
    insert_user("user1", "pass1")
    response = client.post("/token", dict(username="user1", password="pass2"))
...
def test_post_user():
    ...
    user_dict = dict(name="user1", password="secret", mail="a@b.com", role="admin")
    response = client.post("/v1/user", json = user_dict, headers=auth_header )
    #print(response.json())
    assert response.status_code == 201
    assert check_user("user1", "a@b.com") == True
...
def test_post_user_wrong_email():
    auth_header = get_auth_header()
    user_dict = dict(name="user1", password="secret", mail="invalid", role="admin")
    response = client.post("/v1/user", json= user_dict, headers=auth_header)
    #print(response.json())
